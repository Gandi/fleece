            ...
            case opt_field:
                ...
                *tmp = '\0';    /* turn '=' into null terminator */
                ...
                flconf->extra_fields[flconf->extra_fields_len - 1].key =
                    strdup(optarg);
                flconf->extra_fields[flconf->extra_fields_len -
                                     1].key_len = strlen(optarg);
                flconf->extra_fields[flconf->extra_fields_len - 1].value =
                    strdup(tmp);
                flconf->extra_fields[flconf->extra_fields_len -
                                     1].value_len = strlen(tmp);
                break;
            case opt_rsyslog:
...
int main(int argc, char **argv)
{
    ...
    json_t *jsonevent;
    /* propose to make jsonerror global, we could then retreive the error of a function */
        ...
        if (strlen(sendline) == 0) {
            ...
            for (j = 0; j < flconf.extra_fields_len; j++) {
                json_object_set_new(jsonevent, flconf.extra_fields[j].key,
                                    json_string(flconf.extra_fields[j].
                                                value));
            }
...
/* if we are not debugging, maybe let's not care about freeing resources before exit */
#ifdef DEBUG
    if (sockfdsyslog > 0) {
        close(sockfdsyslog);
    }
